...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    // --- NO LONGER NEEDED ---
    // The fetch_batches logic is removed from AJAX.
    // We will load it on the page load like the semester.

    // --- UPDATED --- Fetch exams for a semester AND batch
    if ($action === 'fetch_exams') {
        ...
        if ($sem_id <= 0 || $batch_id <= 0) { 
            echo json_encode(['status' => 'error', 'message' => 'Invalid semester or batch ID']);
            exit;
        }

        $query = "SELECT e.id, e.exam_type, e.exam_date, s.subject_name, e.subject_info_id 
                  FROM examination e 
                  INNER JOIN subject_info s ON e.subject_info_id = s.id 
                  WHERE e.sem_info_id = ? AND e.batch_info_id = ? 
    ...
    // --- UPDATED --- Fetch subjects for a semester AND batch
    if ($action === 'fetch_subjects') {
        ...
        // Assuming subject_info has a batch_id column
        $query = "SELECT id, subject_name 
                  FROM subject_info 
                  WHERE sem_info_id = ? AND batch_id = ?
    ...
    if ($action === 'add_exam') {
        ...
        if ($sem_id <= 0 || $batch_id <= 0 || $subject_id <= 0 || !in_array($exam_type, ['mid1', 'mid2', 'final', 'viva']) || empty($exam_date)) { 
            echo json_encode(['status' => 'error', 'message' => 'Invalid input data']);
            exit;
        }

        $query = "SELECT id FROM examination 
                  WHERE sem_info_id = ? AND batch_info_id = ? AND subject_info_id = ? AND exam_type = ?"; 
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, 'iiis', $sem_id, $batch_id, $subject_id, $exam_type); 
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
        if (mysqli_num_rows($result) > 0) {
            echo json_encode(['status' => 'error', 'message' => 'This exam already exists for the subject, semester, and batch']);
            mysqli_stmt_close($stmt);
            mysqli_close($conn);
            exit;
        }
        ...
        $query = "INSERT INTO examination (sem_info_id, batch_info_id, subject_info_id, exam_type, exam_date) 
                  VALUES (?, ?, ?, ?, ?)"; 
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, 'iiiss', $sem_id, $batch_id, $subject_id, $exam_type, $exam_date); 
    ...
    if ($action === 'edit_exam') {
        ...
        if ($exam_id <= 0 || $sem_id <= 0 || $batch_id <= 0 || $subject_id <= 0 || !in_array($exam_type, ['mid1', 'mid2', 'final', 'viva']) || empty($exam_date)) { 
            echo json_encode(['status' => 'error', 'message' => 'Invalid input data']);
            exit;
        }

        $query = "SELECT id FROM examination 
                  WHERE sem_info_id = ? AND batch_info_id = ? AND subject_info_id = ? AND exam_type = ? AND id != ?"; 
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, 'iiisi', $sem_id, $batch_id, $subject_id, $exam_type, $exam_id); 
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
        if (mysqli_num_rows($result) > 0) {
            echo json_encode(['status' => 'error', 'message' => 'This exam already exists for the subject, semester, and batch']);
                ...
                <div class="flex flex-col md:flex-row md:space-x-4">
                    ...
                    <div class="w-full md:w-1/3 mb-4 md:mb-0">
                        ...
                        <select id="semester" name="semester" class="w-full p-3 border-2 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:outline-none">
                            ...
                            // Do not close connection here yet
                            ?>
                        </select>
                    </div>
                    ...
                    <div class="w-full md:w-1/3 mb-4 md:mb-0">
                        ...
                        <select id="batch" name="batch" class="w-full p-3 border-2 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:outline-none">
                            ...
                            <?php
                            // Using the schema from your other files
                            $batch_query = "SELECT id, batch_start_year, batch_end_year, edu_type FROM batch_info ORDER BY batch_start_year DESC";
                            $batch_result = mysqli_query($conn, $batch_query);
                            while ($row = mysqli_fetch_assoc($batch_result)) {
                                echo "<option value='{$row['id']}'>{$row['batch_start_year']} - {$row['batch_end_year']} ({$row['edu_type']})</option>";
                            }
                            mysqli_close($conn); // Now we can close the connection
                            ?>
                        </select>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            const table = $('#exam-table').DataTable({
                ...
                ordering: false,
                language: {
                    emptyTable: 'Please select a semester and batch to view exams'
                },
                columns: [
                    { data: 'subject_name' },
                    { 
                    ...
                    { 
                        ...
                        render: function(data, type, row) {
                            // Added subject_info_id to data attributes for the edit button
                            return `
                                <button class="edit-btn text-blue-500 transition-all mr-4" data-exam-id="${row.id}" data-subject-id="${row.subject_info_id}" data-subject-name="${row.subject_name}" data-exam-type="${row.exam_type}" data-exam-date="${row.exam_date}" title="Edit Exam">Edit</button>
            ...
            // --- ADDED --- Function to load exams when both are selected
            function loadExams() {
                if (selectedSemId && selectedBatchId) {
                    $('#add-exam-btn').prop('disabled', false); // Enable Add button
                    table.clear().draw();
                        ...
                        success: function(response) {
                            if (response.status === 'success') {
                                ...
                                if (response.exams.length > 0) {
                                    ...
                                    table.draw(); // To show "No data" message if empty
                                }
                            } else {
                    ...
                    $('#add-exam-btn').prop('disabled', true); // Disable if not both selected
                }
            }

            // --- UPDATED --- Semester change
            $('#semester').change(function() {
                selectedSemId = $(this).val();
                table.clear().draw(); 
                loadExams(); // Try to load exams
            });

            // --- UPDATED --- Batch change
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            if (response.subjects.length === 0) {
                                Swal.fire('Info', 'No subjects found for this semester and batch.', 'info');
                                return;
                            }
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                const subjectId = $(this).data('subject-id'); // --- UPDATED ---
